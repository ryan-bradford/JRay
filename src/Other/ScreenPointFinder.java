...
public class ScreenPointFinder {
	...
	public int getWidthPixel(double widthAngle) {// Converts an X Angle to pixel
		double currentFOV = Main.display.FOV; // Gets the FOV
		...
		double angle = TrigFunctions.findLowestAngle(widthAngle - Main.display.xAngle); // Makes sure it is the smallest angle within the 2PI domain
	...
	public int getHeightPixel(double heightAngle) { //Very similar to width pixel
		double currentFOV = Main.display.FOV;
		...
		double angle = TrigFunctions.findLowestAngle(heightAngle - Main.display.yAngle);
